  ...
  return (
    <div style={{ padding: 24, fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        {data ? `${data.status} (${data.nodeEnv})` : "—"}
      </div>
      <div style={{ marginTop: 8 }}>
        <strong>Mode (next click):</strong>{" "}
        {preferCache ? "prefer cache" : "network fetch"}
      </div>

      <p style={{ marginTop: 16, color: "#555" }}>
        Open DevTools → Network and click the button multiple times. You should
        see a request only when mode is "network fetch"; when "prefer cache", it
        returns from cache.
      </p>
    </div>
  );
